...
const createUser = async (req, res) => {
  try {
    ...
    const user = await User.create({ name, email, password: hashed, role: role || 'user', isVerified: true })
    res.status(201).json({ message: 'User created', user: { id: user.id, name: user.name, email: user.email, role: user.role, isVerified: user.isVerified, createdAt: user.createdAt } })
  } catch (err) {
...
module.exports = { createUser, getUsers, updateUser, resetUserPassword }
